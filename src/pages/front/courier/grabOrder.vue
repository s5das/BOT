  ...
  <div class="main" :style="{ height: total_h + 'px' }">
    <div class="condition">
      <div
        ...
        :key="item.id"
        :class="['item', item.id === idOfActivated ? 'isActivated' : '']"
        @click="conditionClickHandler(item)"
      >
    ...
    <div v-if="idOfActivated !== -1" class="condition-detail">
      <div class="choices">
        <div
          @click="choicesClickHandler(item)"
          :class="[
            'choice',
            conditions[idOfActivated].idOfChosen === item.id
              ? 'isActivated'
              : '',
          ]"
          v-for="item in conditions[idOfActivated].choices"
          :key="item.id"
        >
          <div class="label"></div>
...
import { takeOrder } from "@/http/api/courier";
export default {
  name: "grabOrder",
  data() {
    return {
  ...
  methods: {
    ...
    async getOrders() {
      let before_time =
        this.conditions[2].choices[this.conditions[2].idOfChosen].value;
      let ordered_by_reward_desc =
        this.conditions[0].choices[this.conditions[0].idOfChosen].value;
      let pickup_address =
        this.conditions[1].choices[this.conditions[1].idOfChosen].value;
